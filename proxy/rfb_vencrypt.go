...
func Connect(addr string, source net.Conn, target net.Conn) (net.Conn, error) {
	...
	if permittedAuthType[0] != VENCRYPT {
		log.Println("is not VENCRYPT conn")
...
func SecurityHandshake(serverName string, target net.Conn) (net.Conn, error) {
	...
	majVer := byte2int(maj)
	minVer := byte2int(min)
	log.Printf("Server sent VeNCrypt version %v.%v\n", majVer, minVer)
...
func recv(c net.Conn, num int) ([]byte, error) {
	...
	if length != num {
		log.Printf("Incorrect read from socket, wanted %v bytes but got %v. Socket returned\n", num, length)
	}
	return buf, nil
}
